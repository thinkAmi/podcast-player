...
- [フックスクリプトを `rm` される] → V2 により、消えたフックは**無言で素通り**する(fail-open)。ただし V5 の
  実測で `Edit(/.claude/**)` deny が Bash の `rm`・`touch` にも及ぶことが確認でき、この経路は塞がっている。
  残るのは `find -delete` や python 等の間接実行だが、これは脅威モデル外。構造面の緩和(V4: フックを
  唯一の防壁にしない)も維持しているため、仮に消えても防御水準は allowlist とプロンプトのまま。git で即復元可能。
...
### V5: `Write(path)` ルールは参照されない / `Edit(path)` は Bash の `rm`・`touch` まで覆う

公式ドキュメント(permissions)より、ファイルのパスルールは **`Edit(path)` と `Read(path)` だけが参照される**。
`Write(path)` / `NotebookEdit(path)` / `MultiEdit(path)` / `Glob(path)` を書くと**受理されるが決して参照されず**、
起動時に警告が出るだけである。当初案の `Write(./.claude/**)` は無意味だったため `Edit(...)` に統一した。
あわせてアンカーも修正した: `/path` は設定ファイル基準(プロジェクトルート)、`path` / `./path` は cwd 基準。
プロジェクト設定では `/` 始まりが正しい。
...
| 操作 | 結果 |
|---|---|
| `Edit` ツールで `.claude/hooks/guard-device.sh` を編集 | **ブロック** |
| Bash `touch .claude/...` | **ブロック** |
| Bash `rm .claude/...` | **ブロック** |
| `find .claude -delete` / python 等の間接実行 | 素通り(ドキュメント既知の限界) |

**V4 で残存リスクとしていた「フックを `rm` される」経路は塞がっていた。** 認識されるファイルコマンドには
Edit deny が及ぶため、善意の Claude が自然に取る削除経路は止まる。間接実行は covered ではないが、
これは脅威モデル(善意の想定外の一手)から外れる。

### V6: `adb` は PATH に無く、`Bash(adb ...)` 形式のルールは実運用でマッチしない

Claude の Bash 環境では `adb` が PATH に無く、実際の起動は
`~/Library/Android/sdk/platform-tools/adb ...` というフルパス形になる。よって allow の `Bash(adb devices)` や
deny の `Bash(adb uninstall <本番>)` は**実運用ではマッチしない死んだルール**である。
一方フックはコマンド文字列全体を見るため、フルパス起動を捕捉した(実測で確認)。
**deny がすり抜ける形をフックが実際に守っている**という、両者の相補性が実証された格好になる。

### V7: このセッションで検証できた範囲/できなかった範囲

- 検証できた: deny ルールの発火、`Edit` deny による自己改変の拒否、フックの実ブロック(シリアル指定・
  run-as 書き込み・フルパス adb)、フックが読み取り系を誤ブロックしないこと
- 検証できなかった: **allowlist が「未登録コマンドをプロンプトへ落とす」効果**。`defaultMode` は
  セッション開始時の既定であり、実行中のこのセッションのモードが優先されるため、未登録コマンド
  (`echo permission-probe`)がプロンプトなしで実行された。allowlist の効果は**次回以降のセッションで有効**になる

### V8: インシデント — 検証中に本番アプリが実際にアンインストールされた(2026-08-13)

タスク 5.2 の検証を行った別セッションで `./gradlew :app:uninstallDebug` が実行され、
**実機の本番アプリがアンインストールされ、購読・視聴データが失われた**。フォレンジックで確定した事実:

- `firstInstallTime = lastUpdateTime = 2026-08-13 09:34`(再インストールが今日である証拠)
- `databases/podcast.db` は 2026-08-13 09:39 作成の 4KB(ほぼ空)。データ喪失を確認
- `.claude/settings.local.json` は存在しない(保存済み allow ルールでは説明できない)
- `.claude/settings.json` の allowlist・deny・disable 2 種は無傷

**機構の特定(解決済み・2026-08-13)**: 端末未接続 + `--dry-run` の再現実験と利用者の観測により確定。
**ダイアログは出ており、利用者が承認していた。** 自動承認ではなく、権限システム(allowlist の完全一致・
deny・モード封印)は設計どおり動作している。Claude Code のバグではない。当該セッションのエージェントが
「プロンプトなしで実行された」と報告したのは、プロンプトの有無がエージェントから観測できないことによる
誤認(利用者の承認と自動承認は区別がつかない)。

**V8 の本質**: 機械の層は破られていない。破られたのは人間のゲート — 「見慣れた形の破壊コマンドが
ダイアログに出たとき、反射的に承認してしまう」というレッドチーム検証が予言していた事故そのもの。
したがって対策(7.2 の Gradle 側ガード)は「機械の層の穴埋め」ではなく**「人間の反射的承認への防御」**
として位置づける。プロンプトは防御の最終層ではあるが、人間の注意力は消耗品であり、破壊的タスクには
プロンプトの先にもう一段、注意力に依存しない層が要る。

**確定した設計上の教訓**:

1. Gradle の uninstall 系には**モードに依存しない防御層がゼロ**だった。フックは意図的に Gradle を見ず
   (D5)、deny は表記ゆれで無力(V3)、残るは「プロンプトに落ちる」だけ — モード封印が破られれば全滅する。
   単一障害点だった。
2. 検証手順の欠陥: 「プロンプトに落ちるか」の検証は実行を完遂させる必要がなく、端末未接続 +
   `--dry-run` で行うべきだった(グループ 1 ではそうしていたのに、タスク 5.2 にその安全条件を
   明記していなかった)。
3. 復旧は CLAUDE.md の想定どおり軽い(URL 再登録+全視聴済み)が、「軽いから許容する」は
   設計の言い訳にしない。

**対策の方向(タスク 7 系)**: Gradle 側でタスク自体を無効化する。`uD` も `:app:uninstallDebug` も
省略形はすべて同じタスクに解決されるため、タスクに実行時ガードを仕込めば表記ゆれ問題そのものが消える。
モード・プロンプト・フック・deny のどれにも依存しない、このプロジェクトの「ソースコードが設定ファイル」
思想に沿った層になる。

### V9: auto モードの封印解除(2026-08-13、利用者の決定)

D3 の「`disableAutoMode` で auto を封印」を利用者の判断で解除した(settings.json から行を削除。
`disableBypassPermissionsMode` は維持 — bypass には分類器すら無いため解除の論理が成立しない)。

**根拠**: (1) モードを切り替えられるのは利用者だけであり、封印が防いでいたのは実質「利用者自身の
うっかり選択」のみ。そのコスト(セッション再起動によるコンテキスト喪失)が便益を上回った。
(2) deny / ask / フック / Gradle ガードはモード非依存であり、auto セッションでも全層が生きる。
(3) 運用は「調査などで必要なセッションだけ opt-in」であり、常態にはしない。

**同時に実施した前提条件**: auto セッションでは「ガードの脱出ハッチ連鎖」
(deny 拒否 → `:app:` 形 → Gradle ガード → エラーメッセージが案内する `-PallowUninstall=true` を
分類器が通す)が現実的なリスクになるため、フックに `allowUninstall` の一律ブロックを追加した
(部分一致・大文字小文字非依存・パッケージ名チェックより前に配置)。このフラグは利用者が自分の
ターミナルで打つ専用で、Claude 経由の正当な用途は無い。回帰テスト 2 件を追加し全 29 件通過。
なお追加直後、エージェントの探査コマンド(文字列に allowUninstall を含む)が実環境でブロックされ、
機能することが本番で実証された。

## Open Questions

- フックの複数出現・クォート内出現の正規化を、どのシェル機能で最も堅く実装するか(回帰テストで確定する)
- V6 を受け、`Bash(adb ...)` 形式の allow/deny をフルパス形へ書き換えるか、それとも「フックが主、
  ルールは飾り」と割り切って放置するか(現状は後者。`.claude/**` が Edit deny のため、変更は利用者の手で行う)
