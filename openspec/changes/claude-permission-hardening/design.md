...
**未特定(最重要)**: なぜ自動承認されたのか。当該セッションのエージェントは「プロンプトなしで実行された」と
報告したが、プロンプトの有無はエージェントから観測できない(利用者が承認した場合と区別がつかない)。
利用者への確認事項: (a) 確認ダイアログが出て承認したか、(b) そのセッションの権限モード表示。
本当に自動承認だったなら、真の問題は「disableBypassPermissionsMode / disableAutoMode の封印が
効かなかった」ことであり、その機構の特定が先決。
