...
- [x] 5.4 実機での再生スモークテスト(画面消灯継続・ロック画面操作・Bluetooth・着信一時停止)
...
- [x] 8.2 Koverゲート・detekt・lint・全テストのグリーン確認(ローカル+CI)
- [x] 8.3 実機E2E: 実フィード購読→更新→DL→外でのバックグラウンド再生→自動視聴済み→自動削除→フィルター追随の一連確認
