...
## Purpose
Claude Code をこのリポジトリで自律的に走らせる際の権限境界を定める。どのコマンドが無確認で自動実行され、
どれがプロンプト/ブロックに落ちるか、普段使いしている実機(Pixel 7 Pro)の本番アプリ
`dev.thinkami.podcastplayer` のデータをどう守るか、Claude 自身による防御の緩和をどう防ぐか、
そして人間の承認が破られたときに何が最後に止めるかを規定する。
## Requirements
