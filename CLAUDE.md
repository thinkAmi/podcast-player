...
3. **設定画面を作らない。ソースコードが設定ファイル** — 閾値(再生速度の既定値など)は
   コード内定数にする。変更したくなったら定数を書き換えてビルドする。
   (視聴済みは閾値ではなく、プレイヤーが鳴り終えたイベントで確定する)
...
- **PC からの購読登録は `./scripts/add-feed.sh <RSS の URL>`**(AI からは `add-podcast-feed`
  ...
  (契約とクォート処理はスクリプトが持つ)
- **既存購読への過去回の取り込みは
  `./scripts/import-episodes.sh <購読中フィードのURL> <アーカイブXMLのURL>`**(AI からは
  `import-podcast-episodes` スキル経由)。配信側がフィードから落とした過去回を、別途生成した
  アーカイブ XML から同じ番組に合流させる一度きりの操作。取り込み元は記憶せず、以後の更新で
  再取得もしない。**取り違え防止のため、XML の全 item が `<source url>` で申告する出典と
  取り込み先の購読 URL の完全一致をアプリが検証する**(不一致・申告なしは1件も取り込まない)。
  スキルは実行前に「取り込み先・出典宣言・件数」を提示して承認を求める
...
- **その脱出ハッチは AI からは使えない。** `allowUninstall` という文字列を含む Bash コマンドは
  フックが一律ブロックする(大文字小文字を問わない部分一致)。**フラグを使う意図がなく
  文字列に言及しただけのコマンド(`grep allowUninstall` 等)もブロックされる** —
  安全側に倒すための意図的な代償で、不具合ではない。このフラグは利用者が自分の
  ターミナルで打つためのもの
