...
class AdbFeedReceiver : BroadcastReceiver() {
    ...
    override fun onReceive(context: Context, intent: Intent) {
        ...
        CoroutineScope(Dispatchers.IO).launch {
            val outcome = subscribe(repository, feedUrl)
    ...
    private suspend fun subscribe(repository: FeedRepository, feedUrl: String): Outcome =
        try {
            withTimeout(SUBSCRIBE_BUDGET_MS) { repository.subscribe(feedUrl) }
            Outcome(RESULT_SUBSCRIBED, "登録しました: $feedUrl")
        } catch (e: TimeoutCancellationException) {
            Outcome(RESULT_FAILED, "時間内に完了しませんでした(${e.message}): $feedUrl")
        } catch (e: IOException) {
            Outcome(RESULT_FAILED, e.message ?: "フィードを取得できませんでした")
        } catch (e: IllegalStateException) {
            Outcome(RESULT_FAILED, e.message ?: "登録できませんでした")
